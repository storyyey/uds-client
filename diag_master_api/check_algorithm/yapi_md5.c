...
void yapi_MD5Update(yapi_MD5_CTX *context, unsigned char *input, unsigned int inputlen)
{
    ...
    if(inputlen >= partlen)
    {
        ...
        yapi_MD5Transform(context->state, context->buffer);

        for(i = partlen; i+64 <= inputlen; i+=64)
            yapi_MD5Transform(context->state, &input[i]);
...
void yapi_MD5Final(yapi_MD5_CTX *context, unsigned char digest[16])
{
    ...
    yapi_MD5Encode(bits, context->count, 8);
    yapi_MD5Update(context, PADDING, padlen);
    yapi_MD5Update(context, bits, 8);
    yapi_MD5Encode(digest, context->state, 16);
}
