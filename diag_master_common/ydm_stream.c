...
void stream_le_4byte_write(stream_t *sp, unsigned int writelong)
{
    ...
    stream_le_2byte_write(sp, writelong & 0xffff);
}
...
void stream_le_8byte_write(stream_t *sp, unsigned long long writelonglong)
{
    ...
    stream_le_2byte_write(sp, (writelonglong >> 32) & 0xffffffff);
}

void stream_str_write(stream_t *sp, char *p)
{
    while(*p) {
        stream_byte_write(sp, *p++);
    }
}

void stream_nbyte_write(stream_t *sp, unsigned char *p, unsigned int len)
{
    while(len--) {
        stream_byte_write(sp, *p++);
    }
...
void stream_nbyte_read(stream_t *sp, unsigned char *p, unsigned int len)
{
    while(len--) {
        *p++ = stream_byte_read(sp);
    }
}
